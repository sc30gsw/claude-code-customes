...
e-value-management-frontend is the front-end application of ENECHANGE's value management system.

| Category       | Technology               |
| -------------- | ------------------------ |
| Framework      | Next.js 16 / React 19    |
| UI Components  | Mantine 9 / Tailwind CSS |
| State (Server) | SWR                      |
| State (Client) | Jotai                    |
...
pnpm serve            # Serve static output (out/)

# Code Quality (run before committing)
pnpm check            # Run oxlint + oxfmt + tsc together
...
pnpm fmt              # oxfmt format check
pnpm fmt:fix          # oxfmt auto-format
...
pnpm test:e2e:ui      # Playwright E2E tests in UI mode

# Storybook
pnpm storybook              # Start Storybook at http://localhost:6006
pnpm storybook:build        # Build Storybook static files
pnpm test:storybook         # Visual regression tests (Storybook must be running)
pnpm test:storybook:ci      # Visual regression tests for CI
pnpm test:storybook:update  # Update visual regression snapshots

# Code Generation
pnpm generate:api           # Generate API types from openapi.yml
pnpm generate:api:archived  # Generate API types from openapi-archived.yml
pnpm generate:path          # Generate page path types with pathpida

# Diagnostics
pnpm doctor           # react-doctor code quality scan
```
...
1. [**Testing Trophy Strategy**](https://kentcdodds.com/blog/the-testing-trophy-and-testing-classifications): Integration tests (Vitest+MSW, 60%), Unit tests (25%), E2E tests (Playwright, 10%), Visual regression (Storybook, 5%). Test files are co-located with source files (e.g., `@src/features/auth/schemas/login-schema.test.ts`). Reuse MSW handlers from `@src/features/*/mocks/handlers.ts` in Vitest tests. See `@.claude/rules/common/testing.md` for layer-by-layer priorities.
...
8. **Mock Site Branch**: `spike-proposal-mock` is a deploy-only branch for serving the mock site on Cloudflare Pages. Never merge into `main`. See the Mock Site Operations section in README for details.
