...
## Project-specific patterns (aspida + up-fetch)

このプロジェクトでは aspida + up-fetch を使用する。key order は **必ずアルファベット順** にする。

### API層 (mutations.ts): `catch → try` の順

```typescript
// CORRECT: catch → try (アルファベット順)
...
// WRONG: try が先
return Result.tryPromise({
  try: async () => { ... },  // FORBIDDEN: try first
  catch: toApiError,
})
```

### Hook層 (use-\*.ts): `err → ok` の順

```typescript
// CORRECT: err → ok (アルファベット順)
result.match({
  err: (error) => {
...
// WRONG: if/return パターン (return 忘れによるバグの温床)
if (result.isErr()) {
  showError({...})
  // return を忘れると ok ハンドラも実行される
}
showSuccess({...})
```
